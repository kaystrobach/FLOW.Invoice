...
class Invoice
{
    ...
    public function createDocument(ControllerContext $controllerContext = null, bool $hideSinglePrice = false)
    {
        ...
        $view->setLayoutRootPaths(
            ['resource://KayStrobach.Obos/Private/Layouts']
        );
        $view->setTemplateRootPaths(
            ['resource://KayStrobach.Fluege/Private/Templates']
        );
        $view->setPartialRootPaths(
            ['resource://KayStrobach.Fluege/Private/Partials']
        );
        $view->assign(
            'hideSinglePrice',
            $hideSinglePrice
        ...
        $view->assign(
            'qrCodeData',
            $this->getEpcQrCodeData()
        );
        $content = $view->render();
        $now = new DateTime('now');
        $filename = 'Rechnung.' . $this->getNumberPrefix() . $now->format('Y-m-d') . '-' . $this->getNumber() . '.pdf';
        if ($content !== null) {
     ...
     */
    public function getEpcQrCodeData(float $amount = null)
    {
        $data = [
            'BCD', // Service Tag
            '002', // Version
            ...
            $this->receiverBic,  // Empfänger BIX
            $this->receiverName, // Empfänger Name
            $this->receiverIban, // Empfänger IBAN
            'EUR' . number_format($amount ?? $this->total, 2, '.', ''), // Komplett Betrag für die Überweisung
            '',                   // Zweck vierstelliger Code
            '',
            $this->numberPrefix . $this->number
        ];
        return implode('\n', $data);
    }
