(() => {
  // packages/fucodo-form-autosave/index.js
  var FucodoFormAutosave = class extends HTMLElement {
    ...
    _startTimer() {
      ...
      const totalMs = this._seconds * 1e3;
      const indicatorStartMs = Math.max(
        0,
        totalMs - this._indicatorSeconds * 1e3
      );
      if (this._indicator) {
      ...
      this._timer = setTimeout(() => {
        ...
        this._form.requestSubmit ? this._form.requestSubmit() : this._form.submit();
      }, totalMs);
    }
    _clearTimer() {
    ...
    _setIndicator(percent) {
      if (!this._indicator) return;
      if (this._indicatorMode === "progress") {
        this._indicator.value = this._indicator.max * percent / 100;
      } else {
