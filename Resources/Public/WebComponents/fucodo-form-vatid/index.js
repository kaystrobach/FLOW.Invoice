(() => {
    ...
    class VatIdInput extends HTMLElement {
        constructor() {
            ...
            // EU VAT formats (regex-only, no VIES / no checksum)
            // Notes:
            // - Accepts both EL and GR for Greece.
            // - If you provide a slot="country" field, prefix is enforced/auto-added.
            this._patterns = [
                { cc: "AT", re: /^ATU\d{8}$/ },
                { cc: "BE", re: /^BE\d{10}$/ },
                { cc: "BG", re: /^BG\d{9,10}$/ },
        ...
        connectedCallback() {
            ...
            this.countryField = this.querySelector('[slot="country"]'); // optional
            this.valueField = this.querySelector('[slot="value"]');     // required
            ...
            if (initial) {
                ...
                this._syncHiddenAndValidate(norm, /*emit*/ false);
            } else {
                this._syncHiddenAndValidate("", /*emit*/ false);
            }
            ...
            if (this.countryField) {
                this.countryField.addEventListener("change", () => {
                    ...
                    this._syncHiddenAndValidate(enforced, /*emit*/ true);
                });
            }
        }
        ...
        _onInput() {
            const norm = this._normalize(this.visibleInput.value);
            const enforced = this._enforceCountryPrefix(norm);
            // keep typing experience friendly: only pretty-print lightly here
            this._syncHiddenAndValidate(enforced, /*emit*/ true, /*live*/ true);
        }
        ...
        _onBlur() {
            ...
            this.visibleInput.value = this._pretty(enforced);
            this._applyValidationUI(this._validate(enforced), enforced, /*live*/ false);
        ...
        _normalize(value) {
            ...
            return String(value)
                .toUpperCase()
                .replace(/[\s\.\-_/]/g, ""); // remove spaces, dots, dashes, slashes, underscores
        }
        ...
        _pretty(norm) {
            ...
            // If it doesn't look like a country prefix, don't insert spacing
        ...
        _countryFromField() {
            ...
            // allow EL/GR for Greece if user enters GR in select
            return v;
        }
        ...
        _enforceCountryPrefix(norm) {
            ...
            if (!norm) return forced; // start with prefix if user left it empty
            if (norm.startsWith(forced)) return norm;

            // If user typed another prefix (two letters), replace it.
        ...
        _applyValidationUI(res, norm, live) {
            // Bootstrap-ish validity styling without forcing Bootstrap dependency
            this.visibleInput.classList.remove("is-valid", "is-invalid");

            // If empty: no error decoration
            if (res.empty) {
            ...
            if (res.ok) {
                ...
                // During live typing, keep it a bit quieter unless the user clearly has a prefix and some body
